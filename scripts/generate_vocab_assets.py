...
asset schema (list of {"text", "meanings"} per level). Not run at app
build time - run manually whenever the source data needs re-pulling, and
commit the generated JSON under app/src/main/assets/vocab/.
...
def main() -> None:
    ...
    for level in LEVELS:
        ...
        out = [{"text": text, "meanings": meanings} for text, meanings in entries.items()]
